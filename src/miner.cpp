...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
    ...
    LogPrintf("CreateNewBlock(2): txNew.vout[0].scriptPubKey %s\n", HexStr( txNew.vout[0].scriptPubKey ) );
    LogPrintf("CreateNewBlock(2): txNew.vout[1].scriptPubKey %s\n", HexStr( txNew.vout[1].scriptPubKey ) );
    LogPrintf("CreateNewBlock(2): dSCRIPT %s\n", HexStr( dSCRIPT ) );
        // NOTE: unlike in bitcoin, we need to pass PREVIOUS block height here
        CAmount blockReward = nFees + GetBlockSubsidy(pindexPrev->nBits, pindexPrev->nHeight, Params().GetConsensus());

        // Compute regular coinbase transaction.
	CAmount DevAmount = (blockReward * .15);
	CAmount BlockPayout = ( blockReward - (DevAmount) );
        txNew.vout[1].nValue = DevAmount;
        txNew.vout[0].nValue = BlockPayout;
        txNew.vin[0].scriptSig = CScript() << nHeight << OP_0;
        //txNew.vin[1].scriptSig = CScript() << nHeight << OP_0;

    LogPrintf("CreateNewBlock(3): txNew.vout[0].scriptPubKey %s\n", HexStr( txNew.vout[0].scriptPubKey ) );
    LogPrintf("CreateNewBlock(3): txNew.vout[1].scriptPubKey %s\n", HexStr( txNew.vout[1].scriptPubKey ) );
    LogPrintf("CreateNewBlock(3): dSCRIPT %s\n", HexStr( dSCRIPT ) );
