...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    LogPrintf("GetBlockSubsidy():  %d: \n", fSuperblockPartOnly ? nSuperblockPart : nSubsidy);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                          !((pindex->nHeight==0 && pindex->GetBlockHash() == uint256S("0x00000d0ce995bc9f530ed3a9a87721d76222078778f1206edfa3f0527541e342")) ||
                           (pindex->nHeight==0 && pindex->GetBlockHash() == uint256S("0x00000d0ce995bc9f530ed3a9a87721d76222078778f1206edfa3f0527541e342")));
    ...
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > addressUnspentIndex;
    std::vector<std::pair<CSpentIndexKey, CSpentIndexValue> > spentIndex;
        LogPrintf("ConnectBlock(1): block.vtx[0].vout[0].scriptPubKey %s\n", HexStr( block.vtx[0].vout[0].scriptPubKey ) );
        LogPrintf("ConnectBlock(1): block.vtx[0].vout[1].scriptPubKey %s\n", HexStr( block.vtx[0].vout[1].scriptPubKey ) );
        LogPrintf("ConnectBlock(1): dSCRIPT %s\n", HexStr( dSCRIPT ) );
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        ...
        LogPrintf("ConnectBlock(2): block.vtx[0].vout[0].scriptPubKey %s\n", HexStr( block.vtx[0].vout[0].scriptPubKey ) );
        LogPrintf("ConnectBlock(2): block.vtx[0].vout[1].scriptPubKey %s\n", HexStr( block.vtx[0].vout[1].scriptPubKey ) );
        LogPrintf("ConnectBlock(2): dSCRIPT %s\n", HexStr( dSCRIPT ) );

	if (pindex->nHeight > 1) {

		//dSCRIPT << OP_DUP << OP_HASH160 << ParseHex("58D5DA2AD6C07B6819FEA8634015225569D9D196") << OP_EQUALVERIFY << OP_CHECKSIG;
		//if ( ( CScript (ParseHex("0x00") ) + block.vtx[0].vout[1].scriptPubKey) != dSCRIPT )
			
		if ( HexStr(block.vtx[0].vout[1].scriptPubKey) != HexStr(dSCRIPT) )
			return state.DoS(100, error("ConnectBlock() : Dev funds missing (actual=%s dscript=%s)", HexStr(block.vtx[0].vout[1].scriptPubKey), HexStr( dSCRIPT ) ));

		int64_t dAmount = (blockReward * .15);

		if (block.vtx[0].vout[1].nValue < dAmount)
			return state.DoS(100, error("ConnectBlock() : coinbase does not pay enough for development (actual=%d vs required=%d)", block.vtx[0].vout[1].nValue, dAmount));
	}
    ...
    LogPrint("bench", "    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs]\n", nInputs - 1, 0.001 * (nTime4 - nTime2), nInputs <= 1 ? 0 : 0.001 * (nTime4 - nTime2) / (nInputs-1), nTimeVerify * 0.000001);
        LogPrintf("ConnectBlock(3): block.vtx[0].vout[0].scriptPubKey %s\n", HexStr( block.vtx[0].vout[0].scriptPubKey ) );
        LogPrintf("ConnectBlock(3): block.vtx[0].vout[1].scriptPubKey %s\n", HexStr( block.vtx[0].vout[1].scriptPubKey ) );
        LogPrintf("ConnectBlock(3): dSCRIPT %s\n", HexStr( dSCRIPT ) );
    if (fJustCheck)
        return true;
    LogPrintf("ConnectBlock(4): Through fJustCheck" );
    ...
    LogPrintf("ConnectBlock(4): Through GetUndoPos" );
    if (fTxIndex)
    ...
    if (fAddressIndex) {
        ...
        LogPrintf("ConnectBlock(4): block.vtx[0].vout[0].scriptPubKey %s\n", HexStr( block.vtx[0].vout[0].scriptPubKey ) );
        LogPrintf("ConnectBlock(4): block.vtx[0].vout[1].scriptPubKey %s\n", HexStr( block.vtx[0].vout[1].scriptPubKey ) );
        LogPrintf("ConnectBlock4): dSCRIPT %s\n", HexStr( dSCRIPT ) );
    if (fSpentIndex)
    ...
    LogPrint("bench", "    - Callbacks: %.2fms [%.2fs]\n", 0.001 * (nTime6 - nTime5), nTimeCallbacks * 0.000001);
        LogPrintf("ConnectBlock(L): block.vtx[0].vout[0].scriptPubKey %s\n", HexStr( block.vtx[0].vout[0].scriptPubKey ) );
        LogPrintf("ConnectBlock(L): block.vtx[0].vout[1].scriptPubKey %s\n", HexStr( block.vtx[0].vout[1].scriptPubKey ) );
        LogPrintf("ConnectBlock(L): dSCRIPT %s\n", HexStr( dSCRIPT ) );
    return true;
}
